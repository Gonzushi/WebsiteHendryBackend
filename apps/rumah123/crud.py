    ...
    async with aiohttp.ClientSession() as session:
        ...
        for listing in listings:
            try:
                ...
                location = listing.select_one(
                    ".card-featured__middle-section > span"
                ).text
                attributes_text = listing.select(".attribute-text")
                bedrooms = (
                    float(attributes_text[0].text) if len(attributes_text) > 0 else 0
                )
                bathrooms = (
...
async def main(
    base, minPrice, minLandArea, minBuiltupSize, maxLandArea, maxBuiltupSize, max_page=0
):
